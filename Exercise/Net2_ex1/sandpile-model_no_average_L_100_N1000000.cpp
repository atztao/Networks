...
int main()
{
    int sandpile_A[100][100],sandpile_B[100][100]; //定义两个数组进行对称交换——使得其能同时更新-一个判断一个计算更新
    int status[100][100];//扩散状态——当雪崩扩散至此时为1状态标记否则为0
    //int state[100];//雪崩状态——N次实验发生雪崩的概率——状态0 or 1
    int data_s[1000000];//存储不同规模下雪崩大小的数据
    //int data_p[100];//存储雪崩概率的数据
    vector <int> data_t; //Folow T Change

    //使得每次实验沙堆的大小增加到很大很大Add次数越多规模越大
    //方案2:每次加1粒沙子重复10000000000000
        int sandpile_number = 0;
    for (int Add = 0; Add < 1000000; Add++) {
        //int sandpile_number = 100;
        ////每次多增加100粒沙子
        //sandpile_number = sandpile_number + 100;
        ...
        for (int T = 0; T < 100000000; T++) { //弛豫时间T
            int Number = 0;
            //遍历所有格子
